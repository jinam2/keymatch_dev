...
extract($_GET);
switch ($paymethod) {
    case(Card):    // \xEC떊\xEC슜移대뱶
        $pgid = "CARD";
        break;
    case(Account):   // \xEC\x9D\xED뻾 怨꾩쥖 \xEC씠泥\xB4
        $pgid = "ACCT";
        break;
    case(DirectBank): // \xEC떎\xEC떆媛\x84 怨꾩쥖 \xEC씠泥\xB4
        $pgid = "DBNK";
        break;
    case(OCBPoint):  // OCB
        $pgid = "OCBP";
    ...
    case(VCard):    // ISP 寃곗젣
        $pgid = "ISP_";
        break;
    case(HPP):     // \xED쑕\xEB\x8C\xED룿 寃곗젣
        $pgid = "HPP_";
        break;
    case(ArsBill):   // 700 \xEC쟾\xED솕寃곗젣
        $pgid = "ARSB";
        break;
    case(PhoneBill):  // PhoneBill 寃곗젣(諛쏅뒗 \xEC쟾\xED솕)
        $pgid = "PHNB";
        break;
    case(Ars1588Bill):// 1588 \xEC쟾\xED솕寃곗젣
        $pgid = "1588";
        break;
    case(VBank):    // 媛\xEC긽怨꾩쥖 \xEC씠泥\xB4
        $pgid = "VBNK";
        break;
    case(Culture):   // 臾명솕\xEC긽\xED뭹沅\x8C 寃곗젣
        $pgid = "CULT";
        break;
    case(CMS):     // CMS 寃곗젣
        $pgid = "CMS_";
        break;
    case(AUTH):    // \xEC떊\xEC슜移대뱶 \xEC쑀\xED슚\xEC꽦 寃\xEC궗
        $pgid = "AUTH";
        break;
    case(INIcard):   // \xEB꽕\xED떚癒몃땲 寃곗젣
        $pgid = "INIC";
        break;
    case(MDX):     // 紐щ뜳\xEC뒪移대뱶
        $pgid = "MDX_";
        break;
    default:         // \xEC긽湲\xB0 吏遺덉닔\xEB떒 \xEC쇅 異붽\xB0\xEB릺\xEB뒗 吏遺덉닔\xEB떒\xEC쓽 寃쎌슦 湲곕낯\xEC쑝濡\x9C paymethod媛 4\xEC옄由щ줈 \xEB꽆\xEC뼱\xEC삩\xEB떎.
        $pgid = $paymethod;
}

if ($quotainterest == "1") {
    $interest = "(臾댁씠\xEC옄\xED븷遺)";
}
...
class INIData {
    ...
    var $m_sSessionKey;
    //----------------------------
    //IFD \xEC쓳\xEB떟\xED뿤\xEB뜑 \xED븘\xEB뱶
    //----------------------------
    var $m_FlgCrypto;
    var $m_FlgSign;
    var $m_Cmd;
    ...
    var $m_RESULT = array();  //Encrypted \xED븘\xEB뱶 hash table
    ...
    function INIData($request, $request2) {
        ...
        } else if ($this->m_Type == TYPE_REFUND) { //媛\xEC긽怨꾩쥖\xED솚遺\x88(09.08.05)
    ...
    function CheckField() {
        ...
        //媛\xEC긽怨꾩쥖 遺遺꾪솚遺덈룄 遺遺꾪솚遺\x88 濡쒖쭅\xEC뿉 異붽\xB0
        else if (( $this->m_Type == TYPE_REPAY ) || ( $this->m_Type == TYPE_VACCTREPAY )) {
    ...
    function MakeTID() {
        ...
        $datestr_con = substr($datestr, 0, 14) . substr($datestr, 15, 2); //YYYYMMDDHHMMSSxSS 以묎컙\xEC쓽 x媛믪\x9D 踰꾨┝(milli second\xEC쓽 泥ル쾲吏\xB8 \xEC옄由ъ닔)

        mt_srand(getmypid() * mt_rand(1, 999));  //mt_rand \xED븯湲곗쟾\xEC뿉 srand 濡\x9C seed \xEC쟻\xEC슜 , seed key = pid * mt_rand(1,999)
        //pgid + mid + 16\xEC옄由\xAC \xEB궇吏쒕컦 \xEC떆媛\x84 + random_key 4\xEC옄由\xAC (seed\xEC쟻\xEC슜)
    ...
    function MakeEncrypt($INICrypto) {
        ...
        if ($this->m_Type == TYPE_FORMPAY) {
            ...
            //I怨꾩쥖\xEC씠泥\xB4 援\xAD誘쇱\x9D\xED뻾 遺遺꾩랬\xEC냼\xEC떆 怨꾩쥖踰덊샇 怨꾩쥖二쇱꽦紐낆쓣 諛쏆쓬 2011-10-06
            ...
            //怨쇱꽭,鍮꾧낵\xEC꽭 異붽\xB0 2014-07-23 by jung.ks
    ...
    function MakeBody() {
        ...
        //ROOT(INIpay) ROOT瑜\xBC ROOTINFO濡\x9C \xEC닔\xEC젙 2011-05-23
        $root = $xml->add_node("", ROOTINFO);

        if ($this->m_Type == TYPE_SECUREPAY || $this->m_Type == TYPE_RECEIPT) {
            //GoodsInfo
            //\xEC옣諛붽뎄\xEB땲 湲곕뒫 異붽\xB0(2010.04.13)
            //==goodscnt媛 \xEC뾾\xEC쓣 寃쎌슦(\xEC옣諛붽뎄\xEB땲 湲곕뒫\xEC씠 \xEC븘\xEB땺寃쎌슦) 湲곕낯 媛\x92 1濡\x9C \xEC꽕\xEC젙
            ...
            //\xEC옣諛붽뎄\xEB땲 湲곕뒫 異붽\xB0(2010.04.13) 
            //==TX_GOOSCNT\xEB뒗  $tGoodCnt濡\x9C 遺\xED꽣 \xEC엯\xEB젰
            ...
            //\xEC옣諛붽뎄\xEB땲 湲곕뒫 異붽\xB0(2010.04.13) [START]
            //==\xEC옣諛붽뎄\xEB땲 XML \xEC쟾臾\xB8 異붽\xB0
            $iGoodCnt = 1;
            ...
            //\xEC옣諛붽뎄\xEB땲 湲곕뒫 異붽\xB0(2010.04.13) [END]
    ...
    function ParseBody($body, &$encrypted, &$sessionkey) {
        ...
        //PAYMENTINFO
        //湲고\x83吏遺덉닔\xEB떒\xEC씠 paymethod瑜\xBC 二쇱\xA7 \xEC븡\xEC븘 \xEC엫\xEC떆濡\x9C \xEC슂泥\xAD Paymethod濡\x9C \xEB\x8C泥\xB4
        //PG\xEC뿉\xEC꽌 二쇰뒗 PayMethod\xEC\x99 \xEC슂泥쵵aymethod\xEB뒗 媛숇떎!
    ...
    function ParseDecrypt($decrypted) {
        ...
        if ($this->m_Type == TYPE_SECUREPAY || $this->m_Type == TYPE_RECEIPT) {
            //\xEC듅\xEC씤\xEB궇吏\x9C(ApplDate), \xEC듅\xEC씤\xEC떆媛\x84(ApplTime), \xEC듅\xEC씤踰덊샇(ApplNum)\xEB뒗 怨듯넻!!(OCB, \xED쁽湲덉쁺\xEC닔利\x9D \xEC젣\xEC쇅)
            //ISP, CARD\xEB뒗 CARD濡\x9C assign. 怨좉컼/\xEC긽\xEC젏\xEC\x9D ISP媛 萸붿\xA7, CARD媛 萸붿\xA7 \xEC븣寃\x8C 萸먯빞!!!-_-
            if ($this->m_sPayMethod == NM_TX_CARD) {
                ...
                //寃뚯엫臾명솕\xEC긽\xED뭹沅뚯\x9D Multi寃곗젣 媛\xEB뒫
    ...
    // Xpath濡\x9C \xEC븞媛\xEC졇\xEC삩\xEB떎. \xED븳\xEB떖\xEC쓣 \xED뿘吏\xEB엫 \xED뻽\xEB떎!!
