...
include ("../inc/function.php");
	...
	#총 결제액을 구하자(결제된 금액)
	$PRICE_TOTAL_SQL = "SELECT SUM(goods_price) from $jangboo2 WHERE 1=1 $WHERE AND in_check = '1'";
	$PRICE_TOTAL_RESULT = query($PRICE_TOTAL_SQL);
	$PRICE_TOTAL_DATA = happy_mysql_fetch_array($PRICE_TOTAL_RESULT);

	#총 결제액을 구하자(미결제된 금액)
	$PRICE_TOTAL_SQL_N = "SELECT SUM(goods_price) from $jangboo2 WHERE 1=1 $WHERE  ";
