...
for($i = $for_cnt ; $i >=0 ; $i--)
{
	...
	#입금 내역
	$Sql = "SELECT COUNT(*) as ct ,SUM(goods_price) as total_price FROM $jangboo2 WHERE in_check = '1' AND left(DATE_SUB(  curdate(),  INTERVAL $i month  ) ,7) = left(jangboo_date,7) $WHERE";
	...
	#미입 내역
	$Sql = "SELECT COUNT(*) as ct ,SUM(goods_price) as total_price FROM $jangboo2 WHERE in_check = '0' AND left(DATE_SUB(  curdate(),  INTERVAL $i month  ) ,7) = left(jangboo_date,7) $WHERE";
	$Result	= query($Sql);
