...
/* 구인정보가 없는 장부내역은 삭제해주자
$sql = "select * from $jangboo";
$result = query($sql);
while($MONEY = happy_mysql_fetch_assoc($result)) {
	$sql1 = "select count(*) as cnt1 from $guin_tb where number = '$MONEY[links_number]'";
	$result1 = query($sql1);
	$guin_tb2 = happy_mysql_fetch_assoc($result1);

	if ($guin_tb2[cnt1] == "0") {
		$sql2 = "delete from $jangboo where number='$MONEY[number]'";
		echo "구인정보에 없는 $MONEY[number] 번 내역은 삭제합니다.<br>".$sql2."<br>";
		query($sql2);
	}
}
*/
...
{
	gomsg('회원 로그인을 하셔야 합니다.',"./happy_member_login.php");
	exit;
}

$서비스이용항목 = get_uryo_cnt(2);

$TPL->define("리스트", "$skin_folder/my_jangboo_com.html");
$TPL->assign("리스트");
$내용 = &$TPL->fetch();
