<?
	...
	if( $HAPPY_CONFIG['kcb_check_type'] == "ipin" )
	{
		if($_GET['type'] == '')
		{
			...
			$NAME_CHECK_IPIN['returnUrl']	= "https://".$_SERVER["HTTP_HOST"]."/${wys_url}kcb_ipin.php?type=step2_ipin";
...
<?
		...
		else if( $_GET['type'] == 'step2_ipin' )
		{
			...
			$field[6]		= "회원이름";
			$field[7]		= "실명인증한 시간";
			$field[8]		= "나이대를 표시 7 = 20대 , 8 = 30대 , 9 = 40대 ....";
			...
			*/
			if($field[1] != '')
			{
				...
				{
					$field[6] = iconv("euc-kr","utf-8",$field[6]);
				}

				$prefix = ($field[9] == '1') ? "man":"girl";

				echo "
				<script>
					opener.document.chkform.kcb_coinfo1.value='$field[1]';
					opener.document.chkform.kcb_duinfo.value='$field[0]';
					opener.document.chkform.name.value='$field[6]';
					opener.document.chkform.prefix.value='$prefix';
					opener.document.chkform.namecheck_type.value='ipin';
					self.close();
				</script>
				";
				exit;
	...
	else if( $HAPPY_CONFIG['kcb_check_type'] == "hp" )
	{
		...
		{
			$rand				= rand(1234,9999);
			$field[5]			= date("YmdHis").$rand;
			$field[7]			= "테스터";

			$rand2				= rand(1111,1119);
			$field[12]			= "010".$rand2.$rand2;
			$prefix = ($field[9] == '1') ? "man":"girl";
			echo "
			<script>
				alert('데모에서는 휴대폰인증은 그냥 통과 합니다.');
				opener.document.chkform.cl.value='$field[5]';
				opener.document.chkform.name.value='$field[7]';
				opener.document.chkform.prefix.value='$prefix';
				opener.document.chkform.user_hphone.value='$field[12]';
				self.close();
			</script>
			";
			exit;
		...
		if( $_GET['type'] == "" )
		{
			if($demo_lock != '')
			{
				...
				echo "<script>alert('데모에서는 휴대폰인증을 하실 수 없습니다.. ^^');opener.location.replace('index.php'); self.close();</script>";
				exit;
			}

			$NAME_CHECK_HP['returnUrl']			= "https://".$_SERVER["HTTP_HOST"]."/${wys_url}kcb_ipin.php?type=step2_hp";
...
*/
			...
			if($field[0] == 'B000')			//본인인증 성공
			{
				if( $HAPPY_CONFIG['kcb_adultcheck_use'] == '1' )
				{
					...
					else if( $HAPPY_CONFIG['kcb_adultcheck_use'] == 'all' )
					{
						msgclose("성인전용 서비스 입니다..");
						exit;
					}
				}
				...
				if(preg_match("/utf/i",$server_character))
				{
					$field[7] = iconv("euc-kr","utf-8",$field[7]);
				}
				...
				<script>
					...
					opener.document.chkform.prefix.value='$prefix';
					opener.document.chkform.user_hphone.value='$field[12]';
					opener.document.chkform.namecheck_type.value='hp';
					self.close();
				</script>
				";
				exit;
