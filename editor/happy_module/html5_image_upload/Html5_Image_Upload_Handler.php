...
$folder_name = "$upFolder_name/".$upload_id;
...
class UploadHandler
{
	...
	*/
	...
	{
		global $attach_folder_path, $editor_file_path, $main_url, $wys_url;
		global $width_gi_joon, $height_gi_joon, $picture_quality, $swf_logo_file, $swf_logo_position, $thumb_type, $img_type;
		global $upFolder_name, $upload_id, $width_gi_joon_thumb, $height_gi_joon_thumb, $picture_quality, $multi_image_upload_gif;
...
*/
		...
		if( $upload )
		{
			...
			$_FILES['Filedata']['tmp_name'][0]		= HAPPY_EXIF_READ_CHANGE2($_FILES['Filedata']['tmp_name'][0]);

			$tmp_file_name			= happy_microtime(true).mt_rand(0,10000);
			$upload_file_name		= $tmp_file_name.".$ext";
			$upload_file_name_thumb	= $tmp_file_name."_thumb.$ext";

			$folder_name			= "$upFolder_name/".$upload_id;				//tmp 로 업로드해라.

			$img_url_re				= $folder_name."/".$upload_file_name;
			$img_url_re_thumb		= $folder_name."/".$upload_file_name_thumb;

			move_uploaded_file($_FILES['Filedata']['tmp_name'][0], $img_url_re);

			$editor_file_path		= $editor_file_path."/".$upload_file_name;

			/*			애니메이션 GIF 가 아닐 경우에만 썸네일을 만든다.		*/
			if( preg_match("/gif/i",$ext) && $multi_image_upload_gif != "gif썸네일" )		//GIF 이면서 GIF 썸네일 생성방식이 gif애니메이션원본출력 , gif원본출력 일 경우...
			{
				if(  $multi_image_upload_gif == "gif원본출력"  )
				{
					$img_url_re_thumb = imageUploadNewHTML5($img_url_re, $img_url_re_thumb, $width_gi_joon_thumb, $height_gi_joon_thumb, $picture_quality, $thumb_type, 5, "", "" , "0" );
				}
				else if ( $multi_image_upload_gif == "gif애니메이션원본출력" )
				{
					if( !is_animation($img_url_re) )		//애니메이션 아니면 ... 썸네일 생성.
					{
						$img_url_re_thumb = imageUploadNewHTML5($img_url_re, $img_url_re_thumb, $width_gi_joon_thumb, $height_gi_joon_thumb, $picture_quality, $thumb_type, 5, "", "" , "0" );
						imageUploadNew($img_url_re, $img_url_re, $width_gi_joon, $height_gi_joon, $picture_quality, $img_type,'',$swf_logo_file,$swf_logo_position);
					}
					else									//애니메이션은 원본 출력
					{
						$img_url_re_thumb = imageUploadNewHTML5($img_url_re, $img_url_re_thumb, $width_gi_joon_thumb, $height_gi_joon_thumb, $picture_quality, $thumb_type, 5, "", "" , "0" );
			...
			$file->name				= $img_url_re;
			$file->editor_file_path	= $editor_file_path;
			$file->size				= $imagehw['size'];
			$file->type				= $imagehw['mime'];
			$file->url				= $main_url.$wys_url.str_replace("..","",$img_url_re);
			$file->thumbnailUrl		= $main_url.$wys_url."/".str_replace("../","",$img_url_re_thumb);
			//$file->thumbnailUrl		= $main_url.$wys_url.str_replace("..","",$img_url_re);
			$file->deleteUrl		= $main_url.$wys_url."/editor/happy_module/html5_image_upload/Html5_Image_Upload_Handler.php?file=".$img_url_re;
	...
	{
		global $upload_folder, $match_keyword, $upFolder_name, $upload_id;
		...
		{
			$file_path				= $upFolder_name."/".$upload_id."/".$file_name;
			$file_path_thumb		= $upFolder_name."/".$upload_id."/".str_replace(".","_thumb.",$file_name);
			unlink($file_path);
