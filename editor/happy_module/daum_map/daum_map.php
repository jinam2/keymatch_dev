...
$daumMaps_Mark_Name		= "";
$daumMaps_Mark_Width	= "";
$daumMaps_Mark_Height	= "";
foreach( $daum_map_mark_img_array as $key => $value )
{
	$daumMaps_Mark_Name		.= "daumMaps_Mark_Name[".$key."]	= '".$value."'; \r\n";
	$daumMaps_Mark_Width	.= "daumMaps_Mark_Width[".$key."]	= '".$daum_map_mark_width_array[$key]."'; \r\n";
	$daumMaps_Mark_Height	.= "daumMaps_Mark_Height[".$key."]	= '".$daum_map_mark_width_array[$key]."'; \r\n";
}
...
<script type="text/javascript">
	...
	var daumMaps_Key						= '<?=$daum_key?>';
	var daumMaps_Local_Key					= '<?=$daum_local_key?>';

	var daumMaps_Level						= '<?=$daum_map_default_level?>';
	var daumMaps_CenterLat					= '<?=$daum_map_default_lat?>';
	var daumMaps_CenterLng					= '<?=$daum_map_default_lng?>';
	var daumMaps_Width						= '<?=$daum_map_default_width?>';
	var daumMaps_Height						= '<?=$daum_map_default_height?>';
	var daumMaps_minLevel					= '<?=$daum_map_minLevel?>';
	var daumMaps_maxLevel					= '<?=$daum_map_maxLevel?>';
	var daumMaps_draggable					= '<?=$daum_map_draggable?>';
	var daumMaps_scrollwheel				= '<?=$daum_map_scrollwheel?>';
	var daumMaps_disableDoubleClickZoom		= '<?=$daum_map_disableDoubleClickZoom?>';

	var daumMaps_Mark_Name					= Array();
	<?=$daumMaps_Mark_Name?>

	var daumMaps_Mark_Width					= Array();
	<?=$daumMaps_Mark_Width?>

	var daumMaps_Mark_Height				= Array();
	<?=$daumMaps_Mark_Height?>
</script>
...
$(function() {
	$("input:text").keydown(function(evt) {
		if (evt.keyCode == 13)
		{
			...
			else if ( evt.target.id == "daum_search" )
			{
				if ( evt.target.value != "" )
				{
					daum_data_loading();
...
<!-- 엔터키 눌렀을때 서브밋 되는 현상 -->
<link rel="stylesheet" type="text/css" href="css/style_daum_map.css">
...
/* 스킨관계없이 기본 */
	...
	<body>
		...
		<form name='daummap_search_form' target='mapSearchFrame' style='display:none' action='daum_search.php' onSubmit="return false">
			<input type='hidden' name='daum_key' value=''>
			<input type='hidden' name='daum_local_key' value=''>
			<input type='hidden' name='keyword' value=''>
		</form>

		<form name='daummap_search_form2' target='mapSearchFrame' style='display:none' action='daum_search2.php' onSubmit="return false">
			<input type='hidden' name='daum_key' value=''>
			<input type='hidden' name='daum_local_key' value=''>
			<input type='hidden' name='keyword' value=''>
		</form>

		<form name='daummap_form' onSubmit="return false">

		<div style="overflow-y:scroll; height:710px;">
		<table cellspacing="0" cellpadding="0" style="height:60px; background:#555555; width:100%;">
		<tr>
			<td style="color:#fff; padding-left:20px; font-size:14px;"><strong>다음 지도삽입</strong></td>
			<td></td>
		</tr>
		</table>
		...
		<tr>
			<td style="color:#999; padding:20px; line-height:17px;">
				...
				- 다음 지도는 가로,세로사이즈를 가변성있는 100%소스를 지원하지 않습니다.<br>
				- 키워드로 검색시 검색결과는 최대 675개까지만 출력됩니다.(45페이지 x 페이지당 15개)
			</td>
		</tr>
		</table>
		...
		<tr>
			...
			<td>
					...
					<tr>
						<td style="padding:0;"><input type='text' name='daum_search2' id='daum_search2' value='' size=30></td>
						<td  style="padding:0;"><input type='image' src='images/btn_search.gif' onClick="daum_data_loading2()"></td>
					</tr>
					</table>
			</td>
			<th style="width:100px; border-right:1px solid #eaeaea; border-left:1px solid #eaeaea; padding-left:10px; ">가로사이즈</th>
			<td><input type='text' name='daum_width' id='daum_width' value='' size=4 style="color:#0a7ee4; font-family:tahoma; font-size:12px; font-weight:bold; " onKeyUp="daummap_resize()" onKeyPress='if( (event.keyCode<48) || (event.keyCode>57) ) event.returnValue=false;'> px</td>
		</tr>
		<tr>
			<th style="border-right:1px solid #eaeaea; padding-left:10px; ">키워드로검색</th>
			<td>
				<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td style="padding:0;"><input type='text' name='daum_search' id='daum_search' value='<?=$naver_search?>' size=30></td>
					<td style="padding:0;"><input type='image' src='images/btn_search01.gif' onClick="daum_data_loading()"></td>
				</tr>
				</table>
			</td>
			<th style="width:100px; border-right:1px solid #eaeaea; border-left:1px solid #eaeaea; padding-left:10px; ">세로사이즈</th>
			<td style="width:150px;"><input type='text' name='daum_height' id='daum_height' value='' size=4 style="color:#0a7ee4; font-family:tahoma; font-size:12px; font-weight:bold; " onKeyUp="daummap_resize()" onKeyPress='if( (event.keyCode<48) || (event.keyCode>57) ) event.returnValue=false;'> px</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td colspan="3">
				...
				<tr>
					<td><script type="text/javascript">daum_map_mark_loading();</script></td>
		...
		<tr>
			...
			<td colspan="3">
				...
				<tr>
					<td style="font-size:11px; font-family:돋움; background:#fafafa; border-right:1px solid #bdbdc0; height:30px; line-height:30px;"><a href='#onClickZoom' onClick='mapObj.setLevel(parseInt(mapObj.getLevel())+1);daummap_zoom_now();' style="color:#666666; text-decoration:none;">- 축소</a></td>
					...
					<td style="font-size:11px; font-family:돋움; background:#fafafa; border-left:1px solid #bdbdc0;  line-height:30px;"><a href='#onClickZoom' onClick='mapObj.setLevel(parseInt(mapObj.getLevel())-1);daummap_zoom_now();'style="color:#666666; text-decoration:none;">+ 확대</a></td>
				</tr>
				</table>
		...
		<tr>
			...
			<td align="center" valign="middle" style="padding-bottom:10px;" colspan="4">
				...
				<table height="100%" border="1" cellspacing="3" cellpadding="0" bgcolor="#0b6db8">
				<tr bgcolor="white">
					<td align='top'>
						<div align='top' id='mapSearch' width='180' height='410'><iframe id='mapSearchFrame' name='mapSearchFrame' src="daum_search.php" width='180' height='410' frameborder='0'></iframe></div>
					</td>
					<td>
						<script type="text/javascript">daum_map_loading();</script>
					</td>
				</tr>
				</table>
			</td>
		...
		<tr>
			<td align="center" style="border-top:1px solid #eaeaea; padding-top:10px;">
				...
				<tr>
					...
					<td style="padding-left:5px;"><img src="../../img/btn_cancel.png" width="48" height="28" alt="취소" id="btn_cancel" onClick="parent.editor_layer_close()"  style="cursor:pointer;"></td>
				</tr>
				</table>
			</td>
		</tr>
