...
class LanguageConv {
	...
	elseif( $aFF === FF_SIMSUN ) {
	    ...
	    return $this->g2312->str2uni($aTxt);
	}
...
// PUBLIC METHODS
	...
	if( $this->csimcachename != '' ) {
		...
		$diff=happy_mktime()-filemtime($basecsim);
...
class Image {
	...
	*/
	...
	$bbox = @ImageTTFBBox($this->font_size,$aAngle,$this->font_file,str2uni($aTxt));
	if( $bbox === false ) {
...
//('Can not read font file ('.$this->font_file.') in call to Image::GetBBoxTTF. Please make sure that you have set a font before calling this method and that the font is installed in the TTF directory.');
	...
	if( !ereg("\n",$txt) || ($dir>0 && ereg("\n",$txt)) ) {
	    ...
	    ImageTTFText ($this->img, $this->font_size, $dir, $x, $y,
			  $this->current_color,$this->font_file,str2uni($txt));

	    // Calculate and return the co-ordinates for the bounding box
	    $box=@ImageTTFBBox($this->font_size,$dir,$this->font_file,str2uni($txt));
	    $p1 = array();
		...
		ImageTTFText ($this->img, $this->font_size, $dir,
			      $xl, $yl-($h-$fh)+$fh*$i,
			      $this->current_color,$this->font_file,str2uni($tmp[$i]));

		if( $debug  ) {
		    // Draw the bounding rectangle around each line
		    $box=@ImageTTFBBox($this->font_size,$dir,$this->font_file,str2uni($tmp[$i]));
		    $p = array();
		    for($j=0; $j < 4; ++$j) {
			$p[] = $bbox[$j*2]+$xl;
...
// PUBLIC METHODS
	...
	if( $aCacheFileName != "" && USE_CACHE) {
		...
		if( !$aInline ) {
		    ...
		    $diff=happy_mktime()-filemtime($aCacheFileName);
		    if( $diff < 0 )
