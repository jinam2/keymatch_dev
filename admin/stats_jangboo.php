...
function view(){
global $duplication,$jangboo,$car_stats,$pg,$page_print,$now_location_subtitle;
...
END;
 ...
 #SELECT left(jangboo_date,7) as tt FROM `car_jangboo` group by tt
	...
	$sql31 = "SELECT count(*) FROM $jangboo group by left(jangboo_date,7)";
	...
	$sql2 = "select sum(goods_price) from  $jangboo group by left(jangboo_date,7) ";
	$result2 = query($sql2);
	$max_array = array();
	...
	FROM $jangboo group by tt order by number desc limit $view_rows,$pagenum";
	$result = query($sql);
//	print "$sql";
	...
	while ( list($get_date,$total_money,$in_total,$total_count,$in_count) = mysql_fetch_row($result)){
		...
		print "
			<tr>
				...
				<td style='text-align:center;'><a href=stats_jangboo.php?action=detail&number=$get_date style='color:#333'>$get_date</a></td>
...
global $jangboo,$car_stats,$pg,$page_print,$number;
...
print <<<END
<p class='main_title'>결제통계 $DATE[0]년 $DATE[1]월 상세통계</p>
...
END;
	...
	#가장많은 접속을 기준으로 200에 대해서 100%를 줘야 한다.
	$sql2 = "select sum(goods_price) from  $jangboo where left(jangboo_date,7) = '$number'  group by left(jangboo_date,10)  ";
	$result2 = query($sql2);
	$max_array = array();
	while ( list($max_access) = mysql_fetch_row($result2)){
		$max_array[] = $max_access;
	...
	count(number)	as total_count,
	sum(if(in_check = 1,in_check,0)) as in_count
	FROM $jangboo  where left(jangboo_date,7) = '$number' group by tt   order by number desc";
	$result = query($sql);
//	print "$sql";
