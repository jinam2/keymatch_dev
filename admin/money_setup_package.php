...
################################################
	...
	{
		//echo "유료옵션패키지리스트";
		$TemplateFile = "money_setup_package.html";
		...
		$sql = "select * from $job_money_package where 1=1 ".$WHERE2;
		$result = query($sql);
		...
		$sql = "select * from $job_money_package where 1=1 ".$WHERE2." order by number desc limit ".$view_rows.",".$ex_limit;
		$result = query($sql);
		$Datas = array();
		$auto_number = $co;
		while($Data = happy_mysql_fetch_assoc($result))
		{
			...
			$OutData['price'] = number_format($Data['price']);
			//$OutData['uryo_detail'] = kstrcut(uryo_title_convert($Data['uryo_detail']),60,"..");
			$OutData['uryo_detail'] = uryo_title_convert($Data['uryo_detail']);
			$OutData['btn_mod'] = "<a href='money_setup_package.php?mode=modify&number=".$Data['number']."&pay_type=".$pay_type."' class='btn_small_dark'>수정</a>";
			$OutData['btn_del'] = "<a href='money_setup_package.php?mode=del&number=".$Data['number']."&pay_type=".$pay_type."&go_url=".base64_encode($_SERVER['REQUEST_URI'])."'class='btn_small_red'>삭제</a>";
	...
	{
		//echo "유료옵션패키지수정";
		$TemplateFile = "money_setup_package_modify.html";

		$sql = "select * from $job_money_package where number = '".intval($_GET['number'])."'";
		$result = query($sql);
		$Package = happy_mysql_fetch_assoc($result);

		if ( $HAPPY_CONFIG['guin_jump_use'] != "y" )
			$Package['uryo_detail']	= str_replace("guin_jump","",$Package['uryo_detail']);
	...
	else if ( $_GET['mode'] == "mod" )
	{
		...
		if ( is_array($option_array_name) )
		{
			...
			foreach($option_array_name as $v)
			{
				...
				$uryo_detail.= $comma.$v.":".$_POST[$v].":".$_POST["cnt_".$v];
				$comma = ",";
				$i++;
			}
		}

		$sql = "update $job_money_package set ";
		$sql.= "title = '".$_POST['pack_title']."',";
		$sql.= "price = '".$_POST['pack_price']."',";
	...
	{
		$TemplateFile = "money_setup_package_regist.html";
	...
	else if ( $_GET['mode'] == "reg")
	{
		...
		$sql = "insert into $job_money_package set ";
		$sql.= "title = '".$_POST['pack_title']."',";
		$sql.= "price = '".$_POST['pack_price']."',";
		$sql.= "end_day = '".$_POST['end_day']."',";
		$sql.= "comment = '".$_POST['pack_comment']."',";
		...
		msg("패키지상품이 등록되었습니다.");
		go("money_setup_package.php?pay_type=".$pay_type);
		exit;
	...
	{
		$sql = "delete from $job_money_package where number = '".intval($_GET['number'])."'";
		//echo $sql;
		query($sql);
