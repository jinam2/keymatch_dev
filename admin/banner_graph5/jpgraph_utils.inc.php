...
class DateScaleUtils {
    ...
    static function doYearly($aType,$aMinor=false) {
        ...
        if( self::$startday == 1 ) {
            self::$tickPositions[$i++] = happy_mktime(0,0,0,$m,1,$y);
        }
        ++$m;
                    ...
                    if( $aMinor ) {
                        while( $m <= 12 ) {
                            if( !($y == self::$endyear && $m > self::$endmonth) ) {
                                self::$minTickPositions[$j++] = happy_mktime(0,0,0,$m,1,$y);
                            }
                            ++$m;
                        }
                        $m=1;
                    }
                    self::$tickPositions[$i++] = happy_mktime(0,0,0,1,1,$y);
                }
                break;
            case DSUTILS_YEAR2:
                $y=self::$startyear;
                while( $y <= self::$endyear ) {
                    ...
                    for($k=0; $k < 1; ++$k ) {
                        ++$y;
                        if( $aMinor ) {
                            self::$minTickPositions[$j++] = happy_mktime(0,0,0,1,1,$y);
                        }
                    }
                    ++$y;
                }
    ...
    static function doDaily($aType,$aMinor=false) {
        ...
        if( $h == 0 ) {
            self::$tickPositions[$i++] = happy_mktime(0,0,0,$m,$d,$y);
        }
        $t = happy_mktime(0,0,0,$m,$d,$y);
    ...
    static function doWeekly($aType,$aMinor=false) {
        ...
        $week = 1 + (date('z', $thursday) - (11 - date('w', happy_mktime(0, 0, 0, 1, 1, date('Y', $thursday)))) % 7) / 7;
        ...
        if( $daynumber == 1 ) {
            self::$tickPositions[$i++] = happy_mktime(0,0,0,$m,$d,$y);
            $t = happy_mktime(0,0,0,$m,$d,$y) + $hpw;
        }
        else {
            $t = happy_mktime(0,0,0,$m,$d,$y) + $hpd*(8-$daynumber);
        }
    ...
    static function doMonthly($aType,$aMinor=false) {
        ...
        if( $aType == 1 ) {
            if( self::$startday < 15 ) {
                self::$minTickPositions[$j++] = happy_mktime(0,0,0,$m,15,$y);
            }
        }
        ++$m;
        ...
        if(self::$startmonth == self::$endmonth && self::$startyear == self::$endyear && $aType==1 ) {
            self::$tickPositions[$i++] = happy_mktime(0 ,0 ,0, self::$startmonth + 1, 1, self::$startyear);
        }
