...
class GanttGraph extends Graph {
    ...
    function CreateSimple($data,$constrains=array(),$progress=array()) {
        ...
        for( $i=0; $i < $num; ++$i) {
            ...
            // Check if this activity should have a CSIM target占\xBD?
...
class GanttScale {
    ...
    function GetWeekNbr($aDate,$aSunStart=true) {
        ...
        /*-------------------------------------------------------------------------
         ...
         first thursday of year  = 1 + (11 - date("w", happy_mktime(0, 0, 0, 1, 1, date("Y", $day)))) % 7
        ...
        if( $this->hour->iShowLabels ) {
            ...
            for($j=0; $j < $this->GetNumberOfDays(); ++$j, $day += 1,$day %= 7) {
                for($i=0; $i < $n ;++$i, $x+=$width) {
                    ...
                    //$datestamp += $minint*60
                    $datestamp = happy_mktime(date('H',$datestamp),date('i',$datestamp)+$minint,0,
    ...
    function StrokeDays($aYCoord,$getHeight=false) {
        ...
        if( $this->day->iShowLabels ) {
            ...
            for($i=0; $i < $this->GetNumberOfDays(); ++$i, $x+=$daywidth, $day += 1,$day %= 7) {
                ...
                $datestamp = happy_mktime(0,0,0,date("m",$datestamp),date("d",$datestamp)+1,date("Y",$datestamp));
                //$datestamp += SECPERDAY;
