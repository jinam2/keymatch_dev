<? /* Created by SkyTemplate v1.1.0 on 2023/03/30 11:48:16 */
function SkyTpl_Func_2164740742 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
...
<script>
	...
	function search_bar_open(){
		...
		$(".sch_form_detail > ul > li:nth-child(2n)").slideToggle();
	...
	<form method='get' action='guin_list.php' name=a_f_guin style='margin:0'>		
		...
		<div class="hidden sch_form_detail">
			<ul>
				...
				<li>
					<div>
						<b>지역별</b>
						<div class="h_form"><?=$_data['확장지역검색']?></div>
					</div>
				</li>
				<li>
					<div>
		...
		<div class="sch_form_default">	
			...
			<p class="keyword_box">
				...
				<button class="search_color" onClick="document.a_f_guin.submit();" style="background:#<?=$_data['배경색']['모바일_기본색상']?>;">검색하기</button>
			</p>	
