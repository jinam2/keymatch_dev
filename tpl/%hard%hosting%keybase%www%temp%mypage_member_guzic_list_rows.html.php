<? /* Created by SkyTemplate v1.1.0 on 2023/03/23 11:15:49 */
function SkyTpl_Func_1246758479 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
<!-- FORM -->
...
<div>
	<table cellpadding="0" cellspacing="0" style="width:100%; table-layout:fixed; border-collapse: collapse;">
		<tr>
			<td style="width:180px; background:#f3f7f8; text-align:center; border:1px solid #dfdfdf; border-left:0 none; border-top:0 none !important">
			...
			<td style="border:1px solid #dfdfdf; padding:25px 30px; vertical-align:top; text-align:left; border-top:0 none !important">
				...
				<table cellpadding="0"cellspacing="0" style="width:100%; line-height:23px" class="resister_rows">
					<tr>
						...
						<td class="font_14 noto400""><?=$_data['Data']['job_type']?></td>
					</tr>
					<tr>
						<th class="font_14 noto400"">희망지역</th>
						<td class="font_14 noto400""><?=$_data['Data']['job_where']?></td>
					</tr>
					<tr>
						<th class="font_14 noto400"">등록일</th>
						<td class="font_14 noto400""><?=$_data['Data']['regdate']?></td>
					</tr>
					<tr>
						<th class="font_14 noto400"">최종수정일</th>
			...
			<td style="width:220px;; text-align:center; border:1px solid #dfdfdf; border-top:0 none !important">
				<div style="line-height:30px">
					<span style="display:block; letter-spacing:-1px" class="font_14 noto400">
						<span class="font_14 noto400" style="color:#333; letter-spacing:-1px; display:inline-block; width:136px; text-align:center; height:30px; line-height:30px; background:url('./img/main_latest_bg.png') 0 0 no-repeat; color:#<?=$_data['배경색']['기타페이지2']?>">
					...
					<span style="display:block; letter-spacing:-1.2px;" class="font_14 noto400">
						<?=$_data['Data']['grade_money_icon']?> <?=$_data['Data']['grade_money']?>

					</span>
				</div>
			</td>
			<td style="width:200px; border:1px solid #dfdfdf; border-right:0 none; border-top:0 none !important; text-align:center">
				<div style="padding:40px; line-height:24px">
					<span style="display:block; letter-spacing:-1.2px; color:#333;" class="font_14 noto400">
			...
			<tr>
				...
				<!-- 	<a href="document.php?mode=uryo&number=<?=$_data['Data']['number']?>">
					...
					<span style="width:108px; height:28px; line-height:30px; display:inline-block; background:url('img/mypage_per_sth_bg.gif') 0 0 no-repeat; letter-spacing:-1.5px; vertical-align:middle; ">
						<span style="padding-left:23px; color:#666; display:inline-block; margin-top:-2px;" class="font_12 font_dotum"><?=$_data['사용중인옵션']?></span>
					</span>
					<img src="img/bt_myroom_per_ppl.gif" border="0" align="absmiddle" style="cursor:pointer" onClick="document.search_guzic_<?=$_data['doc_extraction_unique_number']?>_<?=$_data['Data']['number']?>_frm.submit()">
					 -->
				</td>
