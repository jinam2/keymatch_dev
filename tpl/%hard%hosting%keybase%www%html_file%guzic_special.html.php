...
function SkyTpl_Func_1710547873 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
<?call_now_nevi('스페셜 인재정보') ?>
		...
		<div class="hidden sch_form_detail" style="background:#f5f5f5;">
			<ul>
				<li>
					<div>
						<b>직종별</b>
						<div class="insert_bracket" style="font-size:0;"><?=$_data['직종검색']?></div>
					</div>
					<div>
						<b>구직종류</b>
						<div><?=$_data['구인타입']?></div>
...
<div class="container_c">
	<div class="sub_tab_menu01">
		<ul>
			<li><a href="/html_file.php?file=guzic_focus.html">포커스 인재정보</a></li>
			<li style="border-top:4px solid #<?=$_data['배경색']['기본색상']?>; box-sizing:border-box;"><a href="/html_file.php?file=guzic_special.html" style="color:#<?=$_data['배경색']['기본색상']?>; font-weight:500;">스페셜 인재정보</a></li>
	...
	<div>
		...
		<div style="padding:40px 0 40px 0" class="sub_special_list2">
			<?newPaging_option('번호양쪽9개노출','구간이동버튼','이전다음버튼','<<','이전','다음','>>') ?>

			<?document_extraction_list('가로3개','세로12개','옵션1','옵션2','스페셜','옵션4','최근등록일순','글자999글자짜름','누락0개','sub_guzic_special_list_rows.html','페이징사용') ?>
