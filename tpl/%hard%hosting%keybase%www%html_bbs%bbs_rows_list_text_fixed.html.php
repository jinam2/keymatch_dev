<? /* Created by SkyTemplate v1.1.0 on 2023/03/14 18:15:14 */
function SkyTpl_Func_3053363211 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
		...
		<tr>
			<td>
				<div class="noto400 font_15 bbs_rows_title ellipsis_line_1" style="height:22px;">
					<?=$_data['BOARD']['bbs_title_re']?><a href="bbs_detail.php?bbs_num=<?=$_data['BOARD']['number']?>&b_category=<?=$_data['_GET']['b_category']?>&id=<?=$_data['_GET']['id']?>&tb=<?=$_data['tb']?>&pg=<?=$_data['pg']?>&links_number=<?=$_data['links_number']?>&start=<?=$_data['start']?>&start=<?=$_data['start']?>"><?=$_data['BOARD']['lock']?><span class="bbs_cate"><?=$_data['BOARD']['b_category_con']?></span><?=$_data['BOARD']['bbs_title_none']?></a>
				</div>
			</td>
