<? /* Created by SkyTemplate v1.1.0 on 2023/03/23 14:06:55 */
function SkyTpl_Func_3422005045 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
<div>
		...
		<tr>
			...
			<td style="padding:17px 15px; text-align:left; border-bottom:1px dashed #dedede; line-height:22px">
				<span style="display:block; letter-spacing:-1px;" class="font_17 noto400">
					<?=$_data['NEW']['bgcolor1']?><a href="guin_detail.php?num=<?=$_data['NEW']['number']?>&pg=<?=$_data['pg']?>&cou=<?=$_data['NEW']['guin_count']?>&clickChk=<?=$_data['clickChk']?>" class="" style="color:#333;">
						<?=$_data['NEW']['title']?>
