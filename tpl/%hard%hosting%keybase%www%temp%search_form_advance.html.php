<? /* Created by SkyTemplate v1.1.0 on 2023/03/27 14:48:45 */
function SkyTpl_Func_30165830 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
<style>
	/* 역세권 자동완성 CSS */
...
<div class="sch_default sch_guin">
	<form  method='get' action='guin_list.php' name=a_f_guin style='margin:0' onSubmit="return false;">
		...
		<div class="hidden sch_form_detail" style="background:#<?=$_data['배경색']['기본색상']?>;">
			<ul>
				<li>
					<div>
						<b>진료과별</b>
						<div class="dual_between_icon" style="font-size:0;"><?=$_data['확장업종검색']?></div>
					</div>
					<div>
						<b>지하철</b>
						<div>
							...
							<div>
								<input type="text" name="underground_search_text" id="underground_search_text" value="<?=$_data['_GET']['underground_text']?>" placeholder="원하시는 진료과 정보를 입력하세요." onKeyUp="startMethodUnder(event.keyCode);" onKeyDown="moveLayerUnder(event.keyCode);" onMouseUp="startMethodUnder(event.keyCode);" onFocus="this.value=''" onBlur="returnValueUnder()" autocomplete="off">
							</div>
				...
				<li>
					<div>
						<b>지역별</b>
						<div class="dual_between_icon" style="font-size:0;"><?=$_data['확장지역검색']?></div>
					</div>
					<div>
						<b>연봉</b>
						<div><?=$_data['연봉검색']?></div>
				...
				<li>
					<div>
						<b>나이제한</b>
						<div><?=$_data['연령검색시작']?><span><?=$_data['연령검색종료']?></span></div>
					</div>					
				</li>
			</ul>
			<a href="javascript:void(0);" onClick="search_bar_open();"><img src="img/close_ico2.png" alt="" /></a>	
		</div>
	</form>
</div>

<!-- <input type="text" name="title_read" id="title_read" value="<?=$_data['title_read']?>" placeholder="검색어를 입력하세요" onfocus="this.placeholder = ''"onblur="this.placeholder = '검색어를 입력하세요'" style="width:930px;"> <button class="h_btn_st1 icon_m uk-icon search_color" style="background:#<?=$_data['배경색']['서브색상']?>;" uk-icon="icon:search; ratio:0.8">검색</button> -->
<? }
?>