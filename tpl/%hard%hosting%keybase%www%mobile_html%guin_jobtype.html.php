<? /* Created by SkyTemplate v1.1.0 on 2023/03/30 11:48:55 */
function SkyTpl_Func_2336020899 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
<style type="text/css">
...
<div class="sub_wrap">
	<h3 class="sub_tlt_st01">
		<b style="color:#<?=$_data['배경색']['모바일_기본색상']?>">유형별 </b>
		<span>초빙정보</span>
	</h3>
	<?include_template('search_form_guin_01.html') ?>
	...
	<div class="sub_guin_list_wrap">
		<h3 class="m_tlt_m_01">
			<strong style="margin-bottom:20px; display:block;"><?=$_data['카테고리명']['1차']?> 초빙정보 리스트  <span style="color:#<?=$_data['배경색']['모바일_기본색상']?>;"><span id="guin_counting" >로딩중</span> 건</span></strong>		
			<p class="h_form sub_list_select">
				<span><?=$_data['초빙정보정렬']?></span>
				<span><?=$_data['초빙정보마감일정렬']?></span>
			</p>
		</h3>
