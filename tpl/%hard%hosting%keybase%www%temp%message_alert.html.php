<? /* Created by SkyTemplate v1.1.0 on 2023/03/07 11:06:11 */
function SkyTpl_Func_3065525131 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
<script>
	var request2;
	...
	function HMessageStartRequest()
	{
		...
		request2.open("GET", "message_ajax.php?receiveid=<?=$_data['message_loginVal']?>&nowTime="+ nowTime , true);
...
<script>
	...
	setInterval("HMessageStartRequest()",<?=$_data['message_setTime']?>);
	//------------------------------------------------------//
</script>
<? }
?>