<? /* Created by SkyTemplate v1.1.0 on 2023/04/20 09:41:38 */
function SkyTpl_Func_1999979725 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
<script language="JavaScript">
function want_del(want_number,jiwon_type) {
	str = "면접제의 한 내역을 삭제하시겠습니까?";
	if ( confirm( str ) )
	{
...
<h3 class="guin_d_tlt_st02">
	 <span style="display:inline-block; vertical-align:middle; width:3px; height:20px; background:#<?=$_data['배경색']['기본색상']?>; margin:0 10px 3px 0;"></span>면접제의 인재정보
</h3>
<div style="border-top:1px solid #ddd;">

	<?newPaging_option('번호양쪽9개노출','구간이동버튼','이전다음버튼','<<','이전','다음','>>') ?>

	<?guin_extraction('총20개','가로1개','제목길이400자','자동','자동','자동','자동','일반','면접제의','mypage_guin_want_list_row.html','','','','최근등록일순') ?>
