...
function SkyTpl_Func_2470580357 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
<div style="border-top:1px solid #dfdfdf;">
	<table cellpadding="0" cellspacing="0" style="width:100%; table-layout:fixed; border-collapse: collapse; background:#<?=$_data['배경색']['서브페이지']?>;">
		<tr>
			<td style="width:107px; text-align:center; border-bottom:1px dashed #dedede">
				<a href="./com_info.php?com_info_id=<?=$_data['GUIN_INFO']['guin_id']?>&guin_number=<?=$_data['GUIN_INFO']['number']?>" target="_blank"><img src="<?=$_data['GUIN_INFO']['logo']?>" alt="기업로고" width="80" height="36" onError="this.src='./img/img_noimage_100x36.jpg'" style="border:1px solid #dedede;"></a>
			</td>
			<td style="padding:17px 15px; text-align:left; border-bottom:1px dashed #dedede; line-height:22px">
				<span style="display:block; letter-spacing:-1px;" class="font_16 noto400">
					<a href='./guin_detail.php?num=<?=$_data['Data']['cNumber']?>&pg=<?=$_data['pg']?>&cou=<?=$_data['NEW']['guin_count']?>&clickChk=<?=$_data['clickChk']?>' style="color:#333"><?=$_data['Data']['guin_title']?></a>
				</span>
			...
			<td style="width:120px; border-bottom:1px dashed #dedede; text-align:center">
				<a href='./guin_detail.php?num=<?=$_data['Data']['cNumber']?>&pg=<?=$_data['pg']?>&cou=<?=$_data['NEW']['guin_count']?>&clickChk=<?=$_data['clickChk']?>' style="color:#333" title="상세보기">
...
<div>
	<table cellpadding="0" cellspacing="0" style="width:100%; table-layout:fixed; border-collapse: collapse;">
		<tr>
			<td class="h_form" style="text-align:center; width:50px; background:#f7f7f7">
				<label class="h-check"><input type="checkbox" name="print_ch[]" value="<?=$_data['Data']['bNumber']?>"><span></span></label>
			</td>
				...
				<table cellpadding="0"cellspacing="0" style="width:100%; line-height:22px" class="resister_rows">
					<tr>
						<th class="font_14 noto400">
							<img src="<?echo happy_image('자동','가로70','세로92','로고사용안함','로고위치7번','기본퀄리티','gif원본출력','img/logo_img.gif','비율대로확대') ?>" border="0" align="absmiddle">
						</th>
						<td class="font_14 noto400" style="text-align:left; padding-left:20px">
							<h4 class="font_16 noto400" style="letter-spacing:-1px; padding-bottom:5px; margin:0;">
							...
							<table cellpadding="0"cellspacing="0" style="width:100%; line-height:22px" class="resister_rows">
								...
								<tr>
									<th class="font_14 noto400">공개한정보</th>
									<td class="font_14 noto400"><?=$_data['Data']['secure']?></td>
								</tr>
							</table>
						</td>
					</tr>
