<? /* Created by SkyTemplate v1.1.0 on 2023/03/29 16:11:25 */
function SkyTpl_Func_2024810741 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
...
<div class="sub_wrap">
	<h3 class="sub_tlt_st01">
		<span style="color:#<?=$_data['배경색']['모바일_기본색상']?>">아르바이트</span>
		<b>초빙정보</b>	
	</h3>
		...
		<form  method='get' action='./bbs_list.php' name=b_search_form1>
			...
			<div class="sch_form_detail"">
				<ul>
					<li>
						<div>
							<b>지역</b>
							<div class="h_form"><?=$_data['지역검색1']?></div>
						</div>
					</li>
					<li>
						<div>
							<b>직종</b>
							<div class="h_form"><?=$_data['b_category_search_form']?></div>
						</div>
						<div>
							<b>급여방식</b>
							<div class="h_form"><?=$_data['text2_select_info']?></div>
						</div>
					</li>
					<li>
						<div>
							<b>필드</b>
							<div class="h_form"><?=$_data['select_name_eq_search']?></div>
						</div>
					</li>
				</ul>
			</div>
			<div class="sch_form_default">	
				...
				<p class="keyword_box">
					...
					<button class="search_color" style="background:#<?=$_data['배경색']['모바일_기본색상']?>;">검색하기</button>
				</p>	
			</div>
		</form>
	</div>
	<div class="sub_albaguin_list_wrap">
		<h3 class="m_tlt_m_01">
			<strong style="margin-bottom:20px; display:block;">아르바이트 초빙정보 <span style="color:#<?=$_data['배경색']['모바일_서브색상']?>;"><?=$_data['total_board_numb']?>건</span></strong>
