<? /* Created by SkyTemplate v1.1.0 on 2023/04/19 15:37:27 */
function SkyTpl_Func_2416398634 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
<?call_now_nevi('스페셜 채용정보') ?>
...
<!------------------------ //새로추가:: 프로그램 ---------------------------->
			...
			<!-- 	<li>
				...
				<!-- <li>
					<div>
						<b>나이제한</b>
						<div style="font-size:0;"><?=$_data['연령검색시작']?><span class="insert_wave"><?=$_data['연령검색종료']?></span></div>
					</div>					
				</li> -->
			</ul>
...
<div class="container_c">
	<div class="sub_tab_menu01">
		<ul>
			...
			<li style="border-top:4px solid #<?=$_data['배경색']['기본색상']?>; box-sizing:border-box;"><a href="/html_file.php?file=guin_special.html" style="color:#<?=$_data['배경색']['기본색상']?>; font-weight:500;">스페셜 초빙정보</a></li>

		</ul>
	</div>
	<div>
		<h3 class="m_tlt">
			<strong>스페셜 초빙 리스트</strong>
			<a href="member_option_pay.php?mode=pay" class="ad_btn"><span>?</span></a>
			<p class="h_form sub_list_select"><span><?=$_data['채용정보정렬']?></span></p>
		</h3>
		<div style="padding:40px 0 40px 0" class="sub_premium_list2">
			<?newPaging_option('번호양쪽9개노출','구간이동버튼','이전다음버튼','<<','이전','다음','>>') ?>

		<?guin_extraction('총8개','가로4개','제목길이200자','자동','자동','자동','자동','스페셜','자동','sub_guin_special_list_rows.html','누락0개','사용함','','최근등록일순') ?>
