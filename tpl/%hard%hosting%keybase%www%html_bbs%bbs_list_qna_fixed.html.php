<? /* Created by SkyTemplate v1.1.0 on 2023/03/23 16:49:28 */
function SkyTpl_Func_905711549 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
<!-- { {TEXT_ALERT} } -->
<div style="width:<?=$_data['B_CONF']['table_size']?>;">
	...
	<div style="border-top:1px solid <?=$_data['B_CONF']['bar_color']?>;">
		...
		<?board_extraction_list('페이지당5개','가로1개','제목길이999자','본문길이0자','공지게시글','bbs_rows_notice04_fixed.html','누락0개','첫페이지만출력') ?>


		<?newPaging_option('번호양쪽4개노출','구간이동버튼','이전다음버튼','<<','이전','다음','>>') ?>


		<?board_extraction_list('페이지당18개','가로1개','제목길이999자','본문길이0자','현재게시판','bbs_rows_list_qna_fixed.html','누락0개','','','0/0') ?>
