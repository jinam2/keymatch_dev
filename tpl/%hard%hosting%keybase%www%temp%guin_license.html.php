<? /* Created by SkyTemplate v1.1.0 on 2023/04/05 15:41:56 */
function SkyTpl_Func_2108861907 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
...
function  send() {
	...
	if (document.form.preference.value == "" ) {
			//alert("보유자격증을 입력하세요. 자격증이 없을 경우 '없음'을 선택하세요")
			//return;
	}

// copy
top.opener.document.regiform.guin_license.value = document.form.preference.value;
// close this window
parent.window.close();
}
...
{
	if (top.opener.document.regiform.guin_license != undefined)
	{
		var licence_val	= top.opener.document.regiform.guin_license.value;
		if (licence_val != "")
		{
			licence_arr	= licence_val.split(", ");
			for(var i=0; i<licence_arr.length; i++)
			{
				for (var j=0; j< document.getElementsByName('p_value').length; j++)
				{
					if (document.getElementById('license_text_'+j).innerHTML == licence_arr[i])
					{
						document.getElementsByName('p_value')[j].checked	= true;
						happy_set(licence_arr[i]);
					}
				}
			}
		}
...
<form name="form" method="post" action="Gi_preference_input_Ok.asp" >
	<div style="padding:10px">
		<table width=100% cellpadding=0 cellspacing=0 border=0 style="border:1px solid #dedede;">
			<tr>
				<td bgcolor="#F4F4F4" align="center" style="padding:10px;">
					<span class="h_form"><input type="text" name="preference" style="width:315px"  readonly ></span> <a href="javascript:send()"><img src="img/bt_guin_popup_reg.gif" align="absmiddle" border="0" style="border-radius:3px;"></a>
					<a href="javascript:reset();"><img src="img/bt_guin_popup_reset.gif" align="absmiddle" border="0" style="border-radius:3px;"></a>
				</td>
			</tr>
		</table>
		<div style="border:1px solid #d2d2d2; margin-top:10px; padding:10px 10px 0 10px" class="aa"><?=$_data['자격증옵션']?></div>
	</div>
</form>
</body>
</html>
