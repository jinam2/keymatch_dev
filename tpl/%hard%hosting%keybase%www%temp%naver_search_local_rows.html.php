<? /* Created by SkyTemplate v1.1.0 on 2023/04/11 10:34:54 */
function SkyTpl_Func_257462506 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
<script>
  var tmPoint_<?=$_data['Count']?>   = new naver.maps.Point(<?=$_data['Data']['mapx']?>,<?=$_data['Data']['mapy']?>);
  var lpoint_<?=$_data['Count']?>   = new naver.maps.TransCoord.fromTM128ToLatLng(tmPoint_<?=$_data['Count']?>);
 </script>
	...
	<td valign='top'>
		...
		<tr>
			<td width='30' align='center'>
				<a href="javascript:happy_map_view_info( <?=$_data['Count']?> ); happy_map_mapObj.setCenter(new naver.maps.LatLng(lpoint_<?=$_data['Count']?>.y,lpoint_<?=$_data['Count']?>.x));happy_map_mapObj.setZoom(8)"><img src='img/map/lpos_<?=$_data['Count']?>_6.png' width="18" height="28" border='0' align='absmiddle' class="png24" __onLoad="happy_map_markAdd(<?=$_data['Count']?>, 'img/map/lpos_<?=$_data['Count']?>_6.png',18 ,28 , '<?=$_data['Data']['mapx']?>', '<?=$_data['Data']['mapy']?>', 0,<?=$_data['Data']['Count']?>, document.getElementById('happy_map_info_window_<?=$_data['Count']?>').innerHTML)" onLoad="happy_map_markAdd_ALL_Call(1000)" ></a>
			</td>
			<td>
...
<input type='hidden' id='happy_map_mark_data5' name='happy_map_mark_data5' value='<?=$_data['Data']['mapx']?>'>
<input type='hidden' id='happy_map_mark_data6' name='happy_map_mark_data6' value='<?=$_data['Data']['mapy']?>'>
