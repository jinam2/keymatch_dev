...
function SkyTpl_Func_2497554090 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
<?
global $BOARD,$GU,$SI;
...
if ($BOARD[text2] == '추후협의'){
	...
	$BOARD[text2_info] = "<span style='letter-spacing:-1px;'>추후협의</span>";
}
else {
...
if ($BOARD[money_in]){
	...
	$BOARD[money_in_info] = "<span style='letter-spacing:-1px;'> 상시채용</span>";
}
else {
...
<div class="alba_listing_03 dp_table">
	<ul>
		<li>
			<span>[<?=$_data['BOARD']['zip_info']?>]</span>
			<a href="bbs_detail.php?bbs_num=<?=$_data['BOARD']['number']?>&b_category=<?=$_data['_GET']['b_category']?>&id=<?=$_data['_GET']['id']?>&tb=<?=$_data['B_CONF']['tbname']?>" class="ellipsis_line1">	<?=$_data['BOARD']['bbs_title_none']?></a>	
		</li>
		<li><span class=""><?=$_data['BOARD']['phone']?></span></li>
		<li><span>~ <?=$_data['BOARD']['money_in_info']?></span></li>
	</ul>
</div>
