<? /* Created by SkyTemplate v1.1.0 on 2023/03/28 16:14:20 */
function SkyTpl_Func_887671494 ($TPL,$DATA,$_index,$_size,$_col) { if ($DATA) $_data=$DATA; else $_data=$GLOBALS;?>
<div style="width:<?=$_data['B_CONF']['table_size']?>;">
	...
	<div class="font_26 noto400" style="color:#222222; padding-top:30px; border-top:1px solid <?=$_data['B_CONF']['bar_color']?>;">
		<span uk-icon="icon:question; ratio:2" style="color:#<?=$_data['배경색']['게시판1']?>; margin-right:10px;"></span><span class="bbs_cate"><?=$_data['BOARD']['b_category']?></span><span style="color:<?=$_data['B_CONF']['down_color']?>;"><?=$_data['BOARD']['bbs_title']?></span>
	</div>
	<!-- 제목 -->

	<!-- 상세카운트정보 sns 글자크기 -->
	<div style="margin-top:30px; border-top:1px solid <?=$_data['B_CONF']['body_color']?>; padding-top:30px;">
		...
		<tr>
			<td>
				...
				<span style="display:<?=$_data['sns_link_btn_display']?>;"><?include_template('bbs_detail_sns_layer_fixed.html') ?></span>
			</td>
	...
	<!-- 본문 -->
	<div style="background:<?=$_data['B_CONF']['detail_color']?>; margin-top:30px;">
		<div class="h_form bbs_attach_file"><?=$_data['BOARD']['attach']?></div>
		<div style="clear:both;"></div>
		<div><?=$_data['BOARD']['img_auto']?></div>
		<div id="ct" class="bbs_detail_faq_box">
			<?=$_data['BOARD']['bbs_review']?>
